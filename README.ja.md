...
> [!NOTE]
> 新規ユーザーが必要なのは `UAV_*` ファイルだけです。Release 内の `ALOS_*`／`Jable*` は旧アップデーター互換用のバイト単位で同一な別名であり、別のアプリではないため重複してダウンロードする必要はありません。
...
4. 「設定」で保存先、画質、ファイル名、同時ダウンロード数、動画ごとのワーカー数、速度制限、AI 字幕、Proxy を調整できます。
...
| 動画ごとのワーカー | 設定範囲は 1–16 です。MissAV は自動的に動画ごと最大 4、全体最大 8 接続に制限され、終盤の再試行も延長されます。SupJav／Hanime1 の直接ダウンロードは最大 4 接続です。Proxy の負荷を下げる場合は小さい値にしてください |
| 画質設定 | 最高、1080p、720p、480p、360p、最低。実際に利用できる画質は配信元によって異なります |
| ファイル名 | 既定は「品番＋完全タイトル」です。「品番のみ」では最初の空白より前（例: `FC2-PPV-1234567`）を使用できます |
...
2. 「設定を表示」で共通の基準日と保存先、または 4 サイトそれぞれの日付とフォルダーを指定できます。画質、ファイル名、動画ごとのワーカー数、バージョン優先度、AI 字幕、Proxy も同じ画面で変更できます。
...
UAV Watcher は 1–168 時間ごと、またはこのコンピューターのローカル時刻で毎日指定時刻に確認できます。既存設定の既定値は引き続き 24 時間です。「今すぐ確認」は重複スケジュールを作成せずに 1 回だけ直ちに実行します。同じ識別番号がカテゴリやサイトをまたいで重複した場合、設定したバージョン優先度に合う候補を残します。番号を確実に識別できない場合は完全に同一の URL だけを重複排除し、推測による統合は行いません。MissAV で一時的に少数のセグメントだけ取得できなかった場合、検証済みの部分は保持され、次回確認時に再開されます。
