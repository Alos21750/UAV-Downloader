...
4. 在“设置”中调整保存位置、画质、文件命名、并发下载数、单片线程数、速度上限、AI 字幕和 Proxy。

| 功能 | 当前行为 |
|---|---|
| 下载队列 | 每项显示状态、进度和速度；队列会持久保存；进行中的任务可停止并优先重新排队，失败任务可单独重试 |
| 并发下载 | 默认同时下载 2 个视频，最多 32 个；AI 字幕使用独立后台队列，不占用视频下载名额 |
| 单片线程 | 设置范围为 1–16；MissAV 会自动限制为每片最多 4 条、全部 MissAV 合计最多 8 条，并延长末段重试；SupJav／Hanime1 直连最多 4 条。使用 Proxy 时可调低以减轻负载 |
| 画质偏好 | 最高、1080p、720p、480p、360p、最低；实际可用画质取决于来源 |
| 文件命名 | 默认为“番号＋完整标题”；可切换为“仅番号”，保留第一个空格前的文字，例如 `FC2-PPV-1234567` |
...
2. 点击“显示设置”调整共同基准日期与文件夹，或为四个网站分别指定日期与保存位置；也可调整画质、文件命名、单片线程、版本优先级、AI 字幕和 Proxy。
...
UAV Watcher 可设置为每 1–168 小时自动检查，或每天按这台电脑的本地时间在指定时刻检查；旧设置仍默认为每 24 小时。点击“立即检查”会马上运行一次，不会建立重复计划。同一番号在不同分类或网站重复出现时，会优先保留符合用户版本偏好的候选项；无法可靠识别番号时仅按完全相同的网址去重，不会猜测合并。MissAV 如果只剩少量分段暂时无法获取，已验证的片段会保留并在下一次检查时续传。
