...
> [!NOTE]
> 每版 Release 只保留 `UAV_Browser.exe`、`UAV_Watcher.exe`、`UAV_Watcher_portable.zip` 與 `UAV_SHA256SUMS.txt`。AI 元件由程式在第一次實際使用時自動取得，不必另外下載或解壓模型包；舊版 ALOS／Jable 檔名也已停止發布。
...
影片進度到 100% 後的合併／MP4 重新封裝，以及字幕辨識的 WAV／ASR 大型暫存，會使用影片儲存資料夾所在磁碟，不再占用 Windows 系統 `%TEMP%`（通常是 C 槽）。

SmartScreen 信譽提醒與 Defender Antivirus 隔離是不同事件。請先閱讀 [Windows 下載與安全驗證](./WINDOWS_SECURITY.md)：核對 `UAV_SHA256SUMS.txt` 與 GitHub provenance；若 Defender 顯示 threat name，請勿直接降低防護設定。

## UAV Browser：瀏覽、挑選、下載

1. 在「瀏覽」選 JableTV、MissAV、SupJav 或 Hanime1，再選分類或輸入關鍵字；Hanime1 另有可組合類型、排序、時間、片長與 240 個標籤的完整篩選器。
2. 勾選多部影片後加入佇列，或直接下載選取項目。
3. 也可在「下載」貼上網址，或從 `.txt` / `.csv` 匯入多個網址。
4. 在「設定」調整儲存位置、畫質、檔案命名、同時下載數、單片執行緒、速度上限、AI 字幕與 Proxy。

| 能力 | 現行行為 |
|---|---|
| 下載佇列 | 每項顯示狀態、進度與速度；佇列會保存；進行中的工作可停止並優先重新排隊，失敗項目可單獨重試 |
| 並行下載 | 預設 2，最多 32 個影片下載；AI 字幕使用獨立背景佇列，不占用下載名額 |
| 單片執行緒 | 設定範圍 1–16；MissAV 會自動限制為每片最多 4 條、全體最多 8 條並延長尾段重試，SupJav／Hanime1 直連最多 4 條。使用 Proxy 時可調低以減少負載 |
| 畫質偏好 | 最高、1080p、720p、480p、360p、最低；實際可用畫質依來源而定 |
| 檔案命名 | 預設「番號＋完整標題」；可改成「僅番號」，保留第一個空白前的文字，例如 `FC2-PPV-1234567` |
...
2. 按「顯示設定」調整共同基準日期與資料夾，或替四個網站分別指定日期與存放位置；也可調整畫質、檔案命名、單片執行緒、版本優先、AI 字幕與 Proxy。
...
| Hanime1 | 272 | 上市／上傳／排行、裏番／泡麵番等 9 種類型、6 種時間、8 種片長與完整 240 標籤 |

UAV Watcher 可設為每 1–168 小時自動檢查，或每天依這台電腦的本地時間在指定時刻檢查；舊設定仍預設每 24 小時。按「立即檢查」會立刻執行一次，不會另外建立重複排程。同一番號跨分類或跨網站重複時，會優先保留符合使用者版本偏好的候選；無法可靠辨識番號時只按完全相同網址去重，不猜測合併。MissAV 若只剩少數分段暫時無法取得，已驗證的片段會保留並在下一次檢查續傳。
...
- 本地英文與台灣繁中翻譯使用固定版本、SHA-256 驗證的 [FuguMT](https://huggingface.co/staka/fugumt-ja-en) 與 [OPUS-MT](https://huggingface.co/Helsinki-NLP/opus-mt-en-zh) INT8 小模型，不使用 Google 或其他免費網路翻譯端點。使用者只需下載 UAV 應用程式；約 **147 MB** 的翻譯元件會在第一次實際產生英文、繁中或三語字幕時由 App 自動取得，之後可離線重用。選擇「不產生」／「日文」或改用 LLM API 都不會取得它。
- 若公司 Proxy／SSL 攔截使自動取得失敗，可把固定的 [`UAV_local_translation_v1.zip`](https://github.com/Alos21750/UAV-Downloader/releases/download/v3.1.0/UAV_local_translation_v1.zip) 原封不動放在 EXE 旁；也支援解壓成旁邊的 `UAV_local_translation_v1` 資料夾。App 只接受尺寸、SHA-256 與逐檔 manifest 全部吻合的元件，無須猜測應放到哪個 AppData 子目錄。
...
Hanime1 支援官方 `watch?v=` 網址、站內搜尋、分頁，以及可組合的排序、類型、上架時間、片長與完整 240 標籤；裏番／泡麵番的精簡列表版型也能正確解析。下載時會即時解析站方簽名 MP4，依畫質偏好選擇可用來源，並支援最多 4 路分段、斷線續傳與單路備援。
