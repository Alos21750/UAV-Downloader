...
> [!NOTE]
> Each release keeps only `UAV_Browser.exe`, `UAV_Watcher.exe`, `UAV_Watcher_portable.zip`, and `UAV_SHA256SUMS.txt`. The app fetches AI components automatically on first use, so users do not need to download or extract model packs. Legacy ALOS and Jable filenames are no longer published.
...
The full-size merge/MP4-remux workspace used after progress reaches 100%, plus large WAV/ASR subtitle scratch files, now stays on the video's selected destination volume instead of Windows `%TEMP%` (normally C:).
...
- Local English and Taiwan Traditional Chinese translation uses pinned, SHA-256-verified [FuguMT](https://huggingface.co/staka/fugumt-ja-en) and [OPUS-MT](https://huggingface.co/Helsinki-NLP/opus-mt-en-zh) INT8 models. It does not use Google or another free network translation endpoint. Users download only the UAV app; the app automatically fetches the approximately **147 MB** translation component when an English, Traditional Chinese, or all-three job first starts, then reuses it offline. Off, Japanese-only, and LLM API jobs do not fetch it.
- If a corporate proxy or SSL interceptor prevents that automatic fetch, place the pinned [`UAV_local_translation_v1.zip`](https://github.com/Alos21750/UAV-Downloader/releases/download/v3.1.0/UAV_local_translation_v1.zip) unchanged beside the EXE. An extracted sibling folder named `UAV_local_translation_v1` is also accepted. The app uses it only after its size, SHA-256, and per-file manifest all verify; no AppData subfolder guesswork is required.
